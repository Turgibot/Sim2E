...
class hdf5_packager(packager):
    ...
    def add_metadata(self, num_pos, num_neg,
        ...
        img = np.full(shape=sensor_size, fill_value=255, dtype='int16')
        for i in range(100):
            ts = self.events_file["events/ts"][:]
            ts_i = int((i/100)*len(ts))
            img_ts = ts[ts_i]
            self.package_image(img, img_ts, i)
    ...
    def set_data_available(self, num_images, num_flow):
        if num_images > 0:
            ...
            self.events_file.attrs['num_images'] = num_images
        if num_flow > 0:
